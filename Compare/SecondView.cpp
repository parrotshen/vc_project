...
IMPLEMENT_DYNCREATE(CSecondView, CEditView)
...
BEGIN_MESSAGE_MAP(CSecondView, CEditView)
...
void CSecondView::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	...
	if( doc->m_is_cursor )
	{
		...
		doc->m_pFirstView->GetEditCtrl().SetSel(nST, nEd+1);
	}
...
void CSecondView::DisplayByHex() 
{
	...
	//取得 Edit 的文字內容
	LPTSTR text = (LPTSTR)doc->m_file2;
	//取得 Edit 的文字內容的長度
	unsigned long len = doc->m_filelen2;
	if(len == 0)
		return;
...
void CSecondView::DisplayByNormal() 
{
	CCompareDoc* doc = (CCompareDoc*)GetDocument();
	SetWindowText( "" );
	//CWaitCursor busy;
...
void CSecondView::SelectOffsetByHex(int offset) 
{
	int Q, R;
	Q = offset / 16;
	R = offset % 16;
...
void CSecondView::SelectOffsetByNormal(int offset) 
{
